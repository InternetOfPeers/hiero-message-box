...
/**
 * Encode data to CBOR format (simplified RFC 8949 implementation).
 * Supports: strings, numbers, booleans, null, undefined, arrays, objects, Buffers.
 * @param {*} data
 * @returns {Buffer}
 */
 ...
 * @returns {{parsed: any, format: string, raw: any}}
 */
...
/**
 * Parse and format a raw HCS message into a human-readable string.
 * @param {Object} msg - Raw HCS message object
 * @param {string|Object} privateKey - RSA PEM or ECIES key object
 * @param {string} encryptionType - 'RSA' or 'ECIES'
 * @returns {Promise<string>}
 */
async function formatMessage(msg, privateKey, encryptionType) {
  ...
  if (parsed && parsed.type === 'HIP-1334_ENCRYPTED_MESSAGE') {
    try {
      const decrypted = await decryptMessage(parsed.data, privateKey);
