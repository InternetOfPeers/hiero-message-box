...
// ---------------------------------------------------------------------------
// encodeCBOR / decodeCBOR roundtrip
// ---------------------------------------------------------------------------
...
describe('encodeCBOR / decodeCBOR roundtrip', () => {
  ...
  describe('unsigned integers', () => {
    test('0 (tiny uint, < 24)', () =>
      expect(decodeCBOR(encodeCBOR(0))).toBe(0));
    test('1', () => expect(decodeCBOR(encodeCBOR(1))).toBe(1));
    test('23 (max tiny uint)', () =>
      expect(decodeCBOR(encodeCBOR(23))).toBe(23));
    test('24 (1-byte uint)', () => expect(decodeCBOR(encodeCBOR(24))).toBe(24));
    test('255 (max 1-byte uint)', () =>
      expect(decodeCBOR(encodeCBOR(255))).toBe(255));
    test('256 (2-byte uint)', () =>
      expect(decodeCBOR(encodeCBOR(256))).toBe(256));
    test('65535 (max 2-byte uint)', () =>
      expect(decodeCBOR(encodeCBOR(65535))).toBe(65535));
    test('65536 (4-byte uint)', () =>
      expect(decodeCBOR(encodeCBOR(65536))).toBe(65536));
    test('1000000 (large 4-byte uint)', () =>
      expect(decodeCBOR(encodeCBOR(1000000))).toBe(1000000));
  });

  describe('negative integers', () => {
    test('-1 (tiny negative)', () =>
      expect(decodeCBOR(encodeCBOR(-1))).toBe(-1));
    test('-24 (most negative tiny)', () =>
      expect(decodeCBOR(encodeCBOR(-24))).toBe(-24));
  });
  ...
  describe('floats / doubles', () => {
    ...
    test('Math.PI', () =>
      expect(decodeCBOR(encodeCBOR(Math.PI))).toBeCloseTo(Math.PI));
  });
  ...
  describe('arrays', () => {
    ...
    test('short array (< 24 items)', () => {
      expect(decodeCBOR(encodeCBOR([1, 'two', true, null]))).toEqual([
        1,
        'two',
        true,
        null,
...
describe('encodeCBOR errors', () => {
  test('throws for Symbol', () => {
    expect(() => encodeCBOR(Symbol('x'))).toThrow(
      'Unsupported CBOR type: symbol'
    );
  });
  test('throws for Function', () => {
    expect(() => encodeCBOR(() => {})).toThrow(
      'Unsupported CBOR type: function'
    );
  });
});
...
describe('decodeCBOR errors', () => {
  ...
  test('throws on unsupported additional info (additionalInfo=28)', () => {
    expect(() => decodeCBOR(Buffer.from([0x1c]))).toThrow(
      'Unsupported additional info: 28'
    );
  });
  test('throws on unsupported major type 6', () => {
    expect(() => decodeCBOR(Buffer.from([0xc0]))).toThrow(
      'Unsupported major type: 6'
    );
  });
  test('throws on unsupported special value in major type 7 (additionalInfo=24)', () => {
    expect(() => decodeCBOR(Buffer.from([0xf8]))).toThrow(
      'Unsupported special value: 24'
    );
  });
});
...
describe('formatMessage with CBOR-encoded messages', () => {
  test('formats a CBOR-encoded encrypted message (cannot decrypt)', async () => {
  ...
  test('formats a CBOR-encoded public key announcement', async () => {
    const msg = makeCborMsg(
      {
...
describe('formatMessage public-key output branches', () => {
  test('omits encryptionType annotation when not provided', async () => {
    ...
    const result = await formatMessage(msg, null, null);
    expect(result).toContain('Public key published by');
    expect(result).not.toMatch(/Public key \(\w+\)/);
    expect(result).toContain('[Seq: 9]');
  });

  test('uses JSON.stringify preview when publicKey is a non-string object', async () => {
    const msg = makeJsonMsg(
      {
        payload: {
          type: 'HIP-1334_PUBLIC_KEY',
    ...
    const result = await formatMessage(msg, null, 'ECIES');
    expect(result).toContain('Public key (ECIES) published by');
    expect(result).toContain('...');
    expect(result).toContain('[Seq: 10]');
  });

  test('shows N/A when publicKey is null', async () => {
    const msg = makeJsonMsg(
      {
        payload: {
          type: 'HIP-1334_PUBLIC_KEY',
